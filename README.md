...
Is a ruby wrapper around the [Processing][processing] code art framework, built using [JRuby][jruby]. <b>Ruby-processing has been superseded by an updated version</b> [JRubyArt][jruby_art] <b>and its standalone alternative</b> [propane][], which both target processing-3.2.2 and support ruby-2.3 syntax. Works on same platforms as vanilla processing (windows, mac, linux) for Android see Yuki Morohoshi [ruboto-processing2][].
