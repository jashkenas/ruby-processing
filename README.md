...
To install jruby-complete use our built in tool (relies on `wget` to download [jruby-complete-1.7.14](http://jruby.org/download))

since ruby-processing-2.5.0 `rp5 setup install` (was `install_jruby_complete`)

If you haven't got `wget` just download jruby-complete-1.7.14 (for ruby-processing-2.6.1)to the vendors folder (then run above tool)
...
### How to build latest development version 2.6.2
See [wiki](https://github.com/jashkenas/ruby-processing/wiki/How-to-build-latest-version)

For current release (version 2.6.1) checkout the "stable" branch and follow the same instructions
